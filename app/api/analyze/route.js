...
Regras gerais:
- Os valores de custo devem ser REALISTAS para França/Europa em 2025
  ...
  try {
    ...
    const costFormatted =
      parsed.costMin === parsed.costMax
        ? parsed.costMin.toLocaleString("pt-BR") + " €"
        : parsed.costMin.toLocaleString("pt-BR") + " – " + parsed.costMax.toLocaleString("pt-PT") + " €";
 
    const validUnits = ["horas", "dias", "semanas", "meses", "anos"];
    const timeUnit  = validUnits.includes(parsed.timeUnit) ? parsed.timeUnit : "meses";
    const timeValue = Number.isInteger(parsed.timeValue) && parsed.timeValue > 0
      ? parsed.timeValue
      : (Number.isInteger(parsed.months) && parsed.months > 0 ? parsed.months : 1);
